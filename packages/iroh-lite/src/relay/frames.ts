...
function encodePingPong(tag: typeof FrameType.Ping | typeof FrameType.Pong, data: Uint8Array): Uint8Array {
...
function encodeDatagrams(
  ...
  batchTag: typeof FrameType.ClientToRelayDatagramBatch | typeof FrameType.RelayToClientDatagramBatch,
  datagrams: Datagrams,
): Uint8Array {
  ...
  if (hasSegmentSize && (!Number.isInteger(segmentSize) || segmentSize < 1 || segmentSize > 0xffff)) {
...
function encodeStatus(status: RelayStatusValue): Uint8Array {
  switch (status.type) {
    ...
    case "same-endpoint-id-connected":
      return concatBytes([frameTag(FrameType.Status), new Uint8Array([RelayStatus.SameEndpointIdConnected])]);
    case "unknown":
